...
class DDPGQNet(nn.Module):
    ...
    def __init__(self, state_dim, action_dim):
        super().__init__()
        self._fc0 = nn.Linear(state_dim + action_dim, 128)
        self._fc1 = nn.Linear(128, 64)
...
class DDPGAgent:
    ...
    def __init__(self, env, qnet_cls, pnet_cls, args):
    ...
    def save(self, path):
        path = os.path.join(path, f'{self._train_ep}.pkl')
        torch.save(self._actor.state_dict(), path)

    def load(self, path, ep):
        path = os.path.join(path, f'{ep}.pkl')
...
def main():
    env = ManyUavEnv(1, True)
    args = DDPGArgs()

    agent = DDPGAgent(env, DDPGQNet, DDPGActor, args)
