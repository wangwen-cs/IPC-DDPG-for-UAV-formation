...
class ManyUavEnv:
    ...
    def get_observation(self):
        observation = []
        for u_i, u in enumerate(self.uav_s):
            rotate_mat = np.array([
            ...
            # the closest uav info
            min_dist = 1e9
            min_uav = None
            for u_other in range(self._uav_cnt):
                if u_other != u_i:
                    p_other = np.array([self.uav_s[u_other].x, self.uav_s[u_other].y])
                    dist = np.linalg.norm(p_other - pos)
                    if dist < min_dist:
                        min_dist = dist
                        min_uav = self.uav_s[u_other]
            other_uav_info = np.array([(min_uav.x - 1000.) / 2000., (min_uav.y - 1000.) / 2000.,
                                       (min_uav.v - 5.) / 10., 0.5 * min_uav.w / np.pi])

            observation.append(
                np.hstack((gps, other_uav_info, (np.array(obstacle_dist) - 1000) / 2000.))
            )
    ...
    def get_reward(self, prev_pos, next_pos):
        ...
        for u in range(self._uav_cnt):
            ...
            for i in range(self._uav_cnt):
                if i != u and np.linalg.norm(next_pos[u] - next_pos[i]) < ManyUavEnv.UAV_COLLISION_R:
                    collision = True
                    break
            if collision:
                result[u] -= 20             # no collision with another uav
